...
specifications_test_filename = 'specs_test.json'
...
def load_experiment_specifications(experiment_directory, test=False):

    filename = os.path.join(experiment_directory, specifications_filename)
    if test:
        filename = os.path.join(experiment_directory, specifications_test_filename)
...
def load_latent_vectors(experiment_directory, checkpoint, test=False):
    ...
    if test:
        ...
        pre = ''
    filename = os.path.join(
        experiment_directory, latent_codes_subdir + pre, checkpoint + ".pth"
    )
...
def get_model_params_dir(experiment_dir, create_if_nonexistent=False, test=False):

    dir = os.path.join(experiment_dir, model_params_subdir)
    if test:
        dir = os.path.join(experiment_dir, model_params_subdir + '_test')
    if create_if_nonexistent and not os.path.isdir(dir):
        os.makedirs(dir)
...
def get_optimizer_params_dir(experiment_dir, create_if_nonexistent=False, test=False):

    dir = os.path.join(experiment_dir, optimizer_params_subdir)
    if test:
        dir = os.path.join(experiment_dir, optimizer_params_subdir + '_test')
    if create_if_nonexistent and not os.path.isdir(dir):
        os.makedirs(dir)
...
def get_latent_codes_dir(experiment_dir, create_if_nonexistent=False, test=False):

    dir = os.path.join(experiment_dir, latent_codes_subdir)
    if test:
        dir = os.path.join(experiment_dir, latent_codes_subdir + '_test')
    if create_if_nonexistent and not os.path.isdir(dir):
        os.makedirs(dir)
